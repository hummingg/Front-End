        ...
        Object.defineProperty(obj, key, {
            ...
            set(newValue) {
                ...
                if (newValue != value) {
                    ...
                    value = newValue;
                    if (key == 'name' || key == 'age') {
                        (document.getElementById(key) as HTMLInputElement).value = newValue;
                    } else if (key == 'people') {
...
class Compiler {
    ...
    compileTemplate(el) {
        let children: HTMLElement[] = Array.from(el.childNodes);    // ES2016
        let fragment = document.createDocumentFragment();
        for (let child of children) {
            let root: DocumentFragment = document.createDocumentFragment();
            if (child.nodeType === 1) {   // 元素结点，要递归
                if (child.getAttribute('h-model')) {  // input
                    root = this.compileModel(child);
                } else if (child.getAttribute('h-for')) {
                    root = this.compileFor(child);
                } else if (child.getAttribute('h-on:click')) {   // button 假设只有按钮有h-on:click，按钮内只有普通文本
                    root = this.compileOnClick(child);
                } else {  // 无指令
                    root = this.compileNoDirective(child);
                }
            } else if (child.nodeType === 3) { // 文本结点
    ...
    compileModel(child: HTMLElement): DocumentFragment {
        let root = document.createDocumentFragment();
        let childInput = child as HTMLInputElement;
        const attrValue: string | null = childInput.getAttribute('h-model');
        if (attrValue === null) {
            root.appendChild(child);
            return root;
        }
    ...
    compileFor(child: HTMLElement): DocumentFragment {
        // 根据隐藏的h-for重新渲染; 已渲染的h-for-ignored有数据，直接丢弃
        if (child.getAttribute('h-for-ignored')) {
            return null;
        }
        let root = document.createDocumentFragment();
        const attrValue: string | null = child.getAttribute('h-for');
        if (attrValue === null) {
            root.appendChild(child);
            return root;
        }
        ...
        for (let i = 0; i < this.$data[iterName].length; i++) {
            ...
            let newNode: HTMLElement = template.cloneNode(false) as HTMLElement;
    ...
    compileOnClick(child: HTMLElement): DocumentFragment {
        let root = document.createDocumentFragment();
        const attrValue = child.getAttribute('h-on:click');
        if (attrValue === null) {
            root.appendChild(child);
    ...
    compileNoDirective(child: HTMLElement): DocumentFragment {
        let root = document.createDocumentFragment();
        root.appendChild(child.cloneNode(false));   // 不复制子节点
        root.firstChild.appendChild(this.compileTemplate(child));
        return root;
    }
    compileText(child: HTMLElement): DocumentFragment {
        let root = document.createDocumentFragment();
